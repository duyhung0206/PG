 ...
 */
class Magestore_Promotionalgift_Model_Mysql4_Sale_Collection extends Mage_Core_Model_Mysql4_Collection_Abstract
{
    public function _construct()
    {
        parent::_construct();
        $this->_init('promotionalgift/sale');
    }
    ...
    public function prepareGiftOrder($range, $customStart, $customEnd)
    {
        ...
        $this->getSelect()->columns(array(
            'gift_order' => 'SUM(gift_total)',
            'order_value' => 'SUM(order_total)',
        ));
        $dateRange = $this->getDateRange($range, $customStart, $customEnd);
        $this->getSelect()->columns(array('range' => $this->_getRangeExpressionForAttribute($range, 'created_at')))
